...
1. At the top of your workflow file, create 4 [environment variables](https://docs.github.com/en/free-pro-team@latest/actions/reference/context-and-expression-syntax-for-github-actions#github-context):
    ...
    - `tag` - This needs to be a unique value each time, as this is used to version the images in the repository. Set `tag` to the `$github.run_number` environment variable.
...
6. To authenticate to the registry, add a step named `Docker login` with the following as the `run` command: `docker login $registryName -u ${{ secrets.ACR_USERNAME }} -p ${{ secrets.ACR_PASSWORD }}`.
...
- The second job (docker login, build and push) doesn't start until the first job (.net build) completes successfully.
